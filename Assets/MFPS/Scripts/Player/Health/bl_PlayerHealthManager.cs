...
public class bl_PlayerHealthManager : bl_PlayerHealthManagerBase
{
    ...
    {
        if (!m_HealthRegeneration || health < 1) return;
        if (health >= RegenerateUpTo) return;

        float projectedHealth = health;
    ...
    [PunRPC]
    void RpcSyncHealth(float newHealth, PhotonMessageInfo info)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public override int GetHealth() => (int)health;

    /// <summary>
    /// 
    /// </summary>
    /// <returns></returns>
    public override int GetMaxHealth() => (int)maxHealth;
