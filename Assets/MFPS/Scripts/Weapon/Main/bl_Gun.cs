...
public class bl_Gun : bl_GunBase
{
    ...
    public void Setup(bool initial = false)
    {
        ...
        if (!initial)
        {
            ...
            {
                numberOfClips = BulletsPerMagazine * numberOfClips;
            }
        }
        CanAiming = true;
    ...
    void Aim()
    {
        if (isAiming && !isReloading)
        {
            ...
            PlayerReferences.weaponSway.SetMotionMultiplier(0.05f);
    ...
    public void SetInifinityAmmo(bool infinity)
    {
        ...
        numberOfClips = AmmoType == AmmunitionType.Bullets ? BulletsPerMagazine * numberOfClips : numberOfClips;
    ...
    public bool OnPickUpAmmo(int bullets, int projectiles, int gunID)
    {
        ...
        {
            //max number of clips
            if (numberOfClips >= maxNumberOfClips) return false;

            numberOfClips += Mathf.FloorToInt(bullets / BulletsPerMagazine);
        ...
        else
        {
            ...
            {
                if (numberOfClips >= BulletsPerMagazine * maxNumberOfClips) return false;

                int oldCount = numberOfClips;
                numberOfClips += bullets;
                numberOfClips = Mathf.Clamp(numberOfClips, 0, BulletsPerMagazine * maxNumberOfClips);
                new MFPSLocalNotification(string.Format("+{0} {1} Bullets", numberOfClips - oldCount, Info.Name));
            }
        }
        UpdateUI();
        return true;
    ...
    public float GetReloadTime()
    {
        if (reloadPer == ReloadPer.Bullet && WeaponAnimation != null)
        {
            ...
            int add = (numberOfClips >= missingBullets) ? missingBullets : numberOfClips;
            float[] bullets = { add };
    ...
    public int GetCompactClips { get { return (numberOfClips / BulletsPerMagazine); } }
    ...
    bool CanReload
    {
        ...
        {
            bool can = false;
            if (bulletsLeft < BulletsPerMagazine && numberOfClips > 0 && controller.State != PlayerState.Running && !isReloading)
            {
                can = true;
            }
    ...
    public bool IsOutOfAmmo() => bulletsLeft <= 0 && numberOfClips <= 0;
